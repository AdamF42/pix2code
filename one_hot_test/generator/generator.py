...
class DataGenerator(Sequence):
    ...
    def __init__(self, img_paths, gui_paths, voc, shuffle=True, batch_size=BATCH_SIZE):
        'Initialization'
    ...
    def context_to_one_hot_indexes(self, partial_sequences):
        ...
        for sequence in partial_sequences:
            sparse_vectors_sequence = []
            for token in sequence:
                sparse_vectors_sequence.append(self.voc.inv_token_lookup[token])
            temp.append(np.array(sparse_vectors_sequence))
    ...
    def create_samples(self, img_paths, gui_paths, voc):
        ...
        for i in range(0, len(gui_paths)):
            ...
            for j in range(0, len(a) - CONTEXT_LENGTH):
                ...
                one_hot_encoding = voc.inv_token_lookup[label]
